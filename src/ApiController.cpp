...
//  An API status code
//
VmbErrorType ApiController::StartContinuousImageAcquisition( const std::string& cameraID, unsigned int nrDrones, const std::string& ip, const std::string& port, char shape )
{
    ...
    if ( VmbErrorSuccess == res )
    {
		...
		std::cout << "port: " << port << "\n";
                ...
                m_pFrameObserver = new FrameObserver(m_pCamera, nrDrones, ip, port, shape);
                // Start streaming
